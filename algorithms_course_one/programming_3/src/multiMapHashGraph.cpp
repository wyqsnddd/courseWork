...
private:
  ...
  std::vector<std::multimap<int, int> >  nodeEdgeHash_;
  ...
  void decrementEdge(int offset){
    std::cout<<"decrement edge: "<< offset<<std::endl;
    m_ = m_ - offset;
  }
  ...
  void insertEdge(int tail, int head ){
    ...
    if(edgeLook==nodeEdgeHash_[nodeLook->second].end()){
      ...
      nodeEdgeHash_[nodeLook->second].insert(
					     std::pair<int,int>(head, getNumberOfEdges())
					     );
...
public:
  multiMapHashGraph(int n, std::string inputFile){
    n_ = n;
    ni_ = 0;
    m_ = 0;
