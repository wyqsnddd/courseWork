...
int main(int argc, char ** argv )
{
  ...
  for(int t = -10000; t<=10000; t++ ){
    for(std::multimap<sampleType, int>::iterator  i = hashTableInt.begin(); 
	i != hashTableInt.end(); 
	i++){
      std::multimap<sampleType, int>::iterator keyFound = hashTableInt.find(t - i->first);
      if(keyFound!=hashTableInt.end()&&keyFound!=i){
	//if(hashTableInt.find(t - i->first)!=hashTableInt.end()){
